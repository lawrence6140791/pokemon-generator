...
| description | string | Yes | species.flavor_text_entries.* | zh-Hant→zh-Hans→DEFAULT_TEXT；移除換行/控制字元（不再 fallback 英文） |
...
8. description 語言優先序：zh-Hant → zh-Hans → DEFAULT_TEXT（不使用英文描述）；若多條符合取第一條
9. 初始顯示固定 pokemon id=25 (皮卡丘) 作為預設卡片（若失敗走一般錯誤/重試流程）
