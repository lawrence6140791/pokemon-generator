...
| animatedGif | string | Optional | sprites.versions['generation-v']['black-white'].animated.front_default | Fallback: front_default → UNKNOWN_IMAGE |
...
4. description 清理：去除 `\n`, `\f` 等特殊換行符 → 空白置換
5. animatedGif 缺失時 fallback 順序：generation-v animated → front_default → UNKNOWN_IMAGE
6. chineseName 選取語言優先序：zh-Hant → zh-Hans → englishName → DEFAULT_TEXT
7. description 語言優先序：同上；若多條符合取第一條
