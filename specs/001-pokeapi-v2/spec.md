...
### Primary User Story
一位使用者開啟頁面後，看到畫面下方有一顆可互動的寶貝球圖示按鈕（唯一互動元素）。使用者點擊（或輕觸）後開始抽取流程並可見按鈕的互動/載入狀態。系統取得一隨機寶可夢資料並顯示為卡片。卡片正面顯示：dream_world SVG 圖（若缺則 front_default；再缺占位圖）、右上角 HP badge、中文名稱（若無則英文；再缺顯示預設文字）、英文名稱（若存在且不同於中文）、屬性徽章（1~2）、官方編號（若後續需要可再加入；目前暫不顯示於 UI 正面以保持簡潔）、底部四項能力數值（HP / 攻擊 / 防禦 / 速度）。背景色依主要屬性映射。點擊卡片翻轉顯示背面：完整描述文字 + 動圖（若有 animated sprite；缺則顯示正面主圖）。使用者可重複點擊寶貝球產生新卡，新卡覆蓋舊卡並重置為正面。
