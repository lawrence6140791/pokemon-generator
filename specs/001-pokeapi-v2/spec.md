...
### Primary User Story
一位使用者開啟頁面後，系統會自動顯示預設的皮卡丘（ID=25）卡片做為起始狀態；卡片正面內容包含：dream_world SVG 圖（若缺則 front_default；再缺占位圖）、右上角 HP badge、中文名稱（若無則英文；再缺顯示預設文字）、英文名稱（若存在且不同於中文）、屬性徽章（1~2）、底部四項能力數值（HP / 攻擊 / 防禦 / 速度），背景色依主要屬性映射。畫面下方提供唯一互動的寶貝球圖示按鈕；使用者可點擊該按鈕抽取下一隻隨機寶可夢（避免連續同 ID），並在載入時於按鈕上看到旋轉動畫。點擊卡片可翻轉並顯示背面：完整描述文字 + 動圖（若有 animated sprite；缺則顯示正面主圖）。再次點擊寶貝球可重複抽取並覆蓋卡片且回到正面。

### Acceptance Scenarios
1. **Given** 使用者初次載入頁面, **When** 頁面完成初始載入, **Then** 自動顯示皮卡丘（ID=25）卡片，其正面內容符合顯示規範（圖片 / 名稱與替代策略 / 屬性 / stats / HP badge）。
...
- 避免重複：不得連續顯示同一隻；若抽到相同 ID 重新抽一次（若再失敗可接受同一隻，避免無限迴圈）。
 - 動圖殘留：抽取新寶可夢後在主圖與背面動圖（若存在）全部預載完成前不更新卡片內容，避免翻面時短暫顯示上一隻寶可夢的動圖。
...
- **FR-005**: 系統 MUST 在卡片正面呈現：主圖（dream_world SVG 優先，其次 front_default，最後占位圖）、HP badge（顯示 hp 數值）、顯示名稱（中文優先 fallback 機制）、英文名稱（若不同於中文）、屬性列表徽章（1~2 個）、四個 stats（HP / 攻擊 / 防禦 / 速度）。
- **FR-006**: 系統 MUST 支援點擊卡片翻轉顯示背面內容（完整敘述、不截斷 + 動圖（若缺則使用主圖）），並允許再次翻轉回正面。
...
- **FR-015**: 系統 MUST 不揭露技術錯誤細節給最終使用者，只顯示一般性失敗訊息。
 ...
 - **FR-020**: 系統 MUST Poké Ball 抽卡按鈕於載入中加速旋轉；非載入時僅在使用者 hover 或 focus-visible 時緩慢旋轉，移出或失焦後停止。
